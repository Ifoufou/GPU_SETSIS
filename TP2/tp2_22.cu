...
// processed by a given CUDA block, by loading them too in the 
// "_block" shared memory (which is now bigger 16x16 => 18x18).
//
...
void conv(uint8_t* img_in, uint8_t* img_out, int width, int height)
{
    ...
    // We extend the size of the array to 18x18 as the convolution will be now
    // performed on the whole 16x16 region.
	...
	// accessed_x, accessed_y will be restraint to [1;16], the block size being
	// configured to 16x16 threads (where idx is in [0;15]).
	signed accessed_y = threadIdx.y+1;
	signed accessed_x = threadIdx.x+1;

	if (row < height && col < width) {
		// NOTE: we can maybe factorize the following "if" statements to optimize 
		// even more the code but I don't want to go further as it gives me 
		// headaches :)
	...
	// We make sure we don't access values out of the frame (see previous 
    // code for more explanations...) and perform the convolution on the
    // 16x16 window.
